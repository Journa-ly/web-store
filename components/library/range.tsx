...
type NativeRangeProps = Omit<React.SelectHTMLAttributes<HTMLInputElement>, 'size'>;

export interface RangeProps extends NativeRangeProps {
  size?: 'lg' | 'md' | 'sm' | 'xs';
}

const Range: React.FC<RangeProps> = ({ size = 'md', className, ...props }) => {
  const classes = clsx('range', `range-${size}`, className);
