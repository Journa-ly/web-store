              ...
              job.id === action.data.id
                ? {
                    ...
                    generation_group_images:
                      action.data.generation_group_images || job.generation_group_images || [],
    ...
    case SET_PIPELINE_JOBS: {
      return {
        ...
        generationsGroups: action.data.map((group) => {
          const currentGroup = state.generationsGroups.find(
            (currentGroup) => currentGroup.id === group.id
          );
          const selectedImage =
            currentGroup &&
            currentGroup.generation_group_images.find((selectedImage) => selectedImage.selected);
          return {
            ...group,
            selected: currentGroup ? currentGroup.selected : false,
            generation_group_images: group.generation_group_images.map((image, index) => ({
              ...image,
              selected:
                selectedImage &&
                currentGroup.generation_group_images.find(
                  (currentImage) => currentImage.uuid === image.uuid
                )
                  ? currentGroup.generation_group_images.find(
                      (currentImage) => currentImage.uuid === image.uuid
                    ).selected
    ...
    case SELECT_IMAGE: {
      return {
        ...
        generationsGroups: state.generationsGroups.map((group) => ({
          ...group,
          generation_group_images: group.generation_group_images
            ? group.generation_group_images.map((image) => ({
                ...image,
                selected:
                  group.id === action.data.generationGroupID
                    ? image.uuid === action.data.imageUUID
                    : image.selected
              }))
            : []
        }))
