...
export default function GenerationListButton({ generation = [], closeAddCharacter }) {
  ...
  const pipelineJobWithInitialImages =
    generation.pipelien_jobs &&
    generation.pipeline_jobs.find((job) => job.initial_images.length > 0);
    ...
    >
      ...
      >
        {generationAvatarImage ? (
          <img
            src={generationAvatarImage.image}
            alt={`generation_${generation.id}`}
            style={{ maxWidth: '100%', borderRadius: '30px', aspectRatio: '1' }}
          />
        ) : (
          <div
            style={{
