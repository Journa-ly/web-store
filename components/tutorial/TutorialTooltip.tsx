...
const TutorialTooltip: React.FC<TutorialTooltipProps> = ({
  ...
  useEffect(() => {
    ...
    if (targetSelector.includes(',')) {
      ...
      target = document.querySelector('.tutorial-target-design') || 
    ...
    if (target instanceof HTMLElement) {
      ...
      target.setAttribute('style', `${originalStyle}; box-shadow: 0 0 0 4px rgba(79, 70, 229, 0.4); border-radius: 4px;`);
    }
    ...
    return () => {
      ...
      if (target instanceof HTMLElement) {
        ...
        target.setAttribute('style', style.replace('box-shadow: 0 0 0 4px rgba(79, 70, 229, 0.4); border-radius: 4px;', ''));
      }
    };
  ...
  const getArrowClass = () => {
    switch (position) {
      ...
      case 'left': return 'tooltip-arrow tooltip-arrow-right';
      case 'right': return 'tooltip-arrow tooltip-arrow-left';
    }
  };
  ...
  const tooltipContent = (
    ...
    >
      <div 
        ref={tooltipRef}
        className={clsx(
          'tutorial-tooltip w-72 rounded-lg bg-indigo-50 shadow-lg border border-indigo-100',
      ...
      >
        ...
        <div className="relative p-4">
          ...
          {/* Title */}
          <h3 className="text-lg font-medium text-indigo-900 mb-1">{title}</h3>
